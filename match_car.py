...
def match_car(plate, car) -> CarMatch:
    make_result = ComparisonResult(compare_field(
        ComparisonValue.MAKE, plate['vin']['make'], car['car']['make']), ComparisonValue.MAKE, plate['vin']['make'], car['car']['make'])
    model_result = ComparisonResult(compare_field(
        ComparisonValue.MODEL, plate['vin']['model'], car['car']['model']), ComparisonValue.MODEL, plate['vin']['model'], car['car']['model'])
    year_result = ComparisonResult(compare_field(
        ComparisonValue.YEAR, plate['vin']['year'], car['car']['years']), ComparisonValue.YEAR, plate['vin']['year'], car['car']['years'])
    ...
    if car['color']['probability'] < 0.5 or plate['vin']['color']['abbreviation'] == 'UNK':
        color_result = ComparisonResult(
            MatchResult.INDETERMINATE, ComparisonValue.COLOUR, plate['vin']['color']['name'], car['color']['name'])
    else:
        color_result = ComparisonResult(compare_field(
            ComparisonValue.COLOUR, plate['vin']['color']['name'], car['color']['name']), ComparisonValue.COLOUR, plate['vin']['color'].name, car['color']['name'])
